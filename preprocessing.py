...
class preprocessing:
    ...
    def make_word_dict(self):
        ...
        for i in range(self.dataframe.shape[0]):
            # word_lst = self.dataframe.review[i].lower().split()
            
            sentence = self.dataframe.review[i].lower()
            sentence = re.sub('[\d]+', 'NUMBER', sentence)
            sentence = re.sub('[^A-Za-z\s]', '', sentence)
            sentence = re.sub('[NUMBER]+', 'NUMBER', sentence)
            word_lst = sentence.split()

            if self.dataframe.label[i] == 1:
                for ele in word_lst:
    ...
    def make_review_lst(self, del_word_lst):
        ...
        new_review_lst = []
        for sentence in tqdm(self.dataframe.review):
